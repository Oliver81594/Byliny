...
function zobraz_vysledky(){
    zmaz();
    for (var v = 0; v < vysledky.length; v++){
...
function spracuj(event){             //funkcia spracuj caka kedy uzivatel stlaci enter, nasledne spusti funckiu hladaj
    //if(event.key == "Enter"){
        var string = vyhladavac.value.replace(/[\n\r]/g, ''); //odstrani nove riadky
        var kluc = new RegExp(string, "i");       //RegExp je funkcia, ktora zmeni string na regular expression. Icko znamena, ze nezalezi ci je prve pismeno velke alebo male
        hladaj();
        vysledky = [];
    //}
}
